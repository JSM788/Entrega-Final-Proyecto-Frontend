...
import map from '../../../shared/assets/icons/map.svg?url';
import calendario from '../../../shared/assets/icons/calendario.svg?url';
import './SearchBar.css'; // Asegúrate de importar el CSS
...
export const SearchBar = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (value === '') {
      ...
      const filtered = cities.filter((city) => `${city.countryName}, ${city.cityName}`.toLowerCase().includes(value.toLowerCase()));
  ...
  const handleCitySelect = (city) => {
    setInputValue(`${city.countryName}, ${city.cityName}`);
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Tab' && filteredCities.length > 0) {
      ...
      setInputValue(`${firstCity.countryName}, ${firstCity.cityName}`);
  ...
  const handleBlur = () => {
    const matchedCity = cities.find(
      (city) => `${city.countryName}, ${city.cityName}`.toLowerCase() === inputValue.toLowerCase(),
    );
    if (matchedCity) {
              ...
              {filteredCities.length > 0 && (
                <div className="absolute bg-white p-1 shadow-lg rounded-lg w-full mt-1 max-h-40 overflow-y-auto z-20 transition-all ease-in-out duration-300">
                  {filteredCities.map((city) => (
                    ...
                    >
                      {highlightMatch(`${city.countryName}, ${city.cityName}`, inputValue)}
                    </div>
                  ))}
                </div>
              )}
