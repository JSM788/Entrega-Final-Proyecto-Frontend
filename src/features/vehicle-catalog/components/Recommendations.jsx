      ...
      <CardBody className="w-full md:w-2/3 pl-4">
        ...
        <div className="flex items-start">
          <Typography variant="small" color="gray" className="mb-1">
            Trasmisión: {vehicle.transmission}
          </Typography>
        </div>
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch(`${baseUrl}/api/products/paged?page=0&size=2`);
    ...
    <div className="bg-[#DEECEC] p-4 m-5 mx-auto">
      ...
      >
        {vehicles?.map((vehicle) => (
          <RecommendationCard key={vehicle.productId} vehicle={vehicle} />
        ))}
      </div>
    </div>
  );
