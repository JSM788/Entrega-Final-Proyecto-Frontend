...
import MultiSelectCharacteristics from '../../../shared/ui/MultiSelectCharacteristics';
...
const AddVehicleForm = ({
  ...
  const [vehicleData, setVehicleData] = useState({
    ...
    brand_name: "",
    categoryId: "",
    characteristicIds: [], // Changed from characteristics
    itemProducts: [
      {
        nro_serial: "",
        color: "",
  ...
  const [allCharacteristics, setAllCharacteristics] = useState([]); // Nuevo estado para todas las características
  ...
  useEffect(() => {
    ...
    const fetchCharacteristics = async () => {
      try {
        const baseUrl = import.meta.env.VITE_API_BASE_URL;
        const response = await fetch(`${baseUrl}/api/characteristics`);
        if (response.ok) {
          const data = await response.json();
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Hubo un problema al cargar las características.",
          showConfirmButton: false,
          timer: 2000,
        });
      }
  ...
  const validateForm = () => {
    ...
    if (vehicleData.characteristicIds.length === 0) {
      errors.characteristicIds = "Debe seleccionar al menos una característica.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const newVehicle = {
      ...
      characteristicIds: vehicleData.characteristicIds,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <MultiSelectCharacteristics
                  ...
                  selectedCharacteristicIds={vehicleData.characteristicIds}
                  onSelectionChange={(newIds) => setVehicleData({ ...vehicleData, characteristicIds: newIds })}
                />
                {formErrors.characteristicIds && (
                  <p className="text-red-500 text-sm mt-1">
                    {formErrors.characteristicIds}
                  </p>
                )}
